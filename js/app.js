/* =========================================================
   ...
   ========================================================= */
const WA_NUMBER = "2349163341233"; // ← replace with the brand's WhatsApp number
const CURRENCY = "₦";
...
/* =========================================================
   ...
   ========================================================= */
const CATS = [
  { n: "Lace", c: "Delicate & detailed", fabric: "lace", col: "Wine" },
  { n: "Sequins", c: "Shine & statement", fabric: "sequin", col: "Gold" },
  { n: "Velvet", c: "Rich & tactile", fabric: "velvet", col: "Royal" },
  { n: "Silk", c: "Fluid & luxe", fabric: "silk", col: "Emerald" },
  { n: "Brocade", c: "Woven heritage", fabric: "brocade", col: "Gold" },
  { n: "Ankara", c: "Bold & everyday", fabric: "ankara", col: "Terracotta" },
  { n: "Chiffon", c: "Light & airy", fabric: "chiffon", col: "Blush" },
  { n: "Embroidery", c: "Ornate & bridal", fabric: "embroidery", col: "Champagne" }
];
function renderCats() {
  document.getElementById("catGrid").innerHTML = CATS.map((x, i) => `
    <a class="cat" href="shop.html?type=${encodeURIComponent(x.n)}">
      ${swatchHTML(x.fabric, x.col)}
      <div class="cat-veil"></div>
      <span class="cat-num">${String(i + 1).padStart(2, "0")}</span>
      <div class="cat-label"><div class="n">${x.n}</div><div class="c">${x.c}</div></div>
    </a>`).join("");
}
...
/* =========================================================
   ...
   ========================================================= */
function renderEdit() {
  const pick = [PRODUCTS[0], PRODUCTS[3], PRODUCTS[12], PRODUCTS[8]];
...
function renderIG() {
  const set = [PRODUCTS[0], PRODUCTS[3], PRODUCTS[12], PRODUCTS[8], PRODUCTS[5], PRODUCTS[9], PRODUCTS[1], PRODUCTS[13], PRODUCTS[7], PRODUCTS[4], PRODUCTS[10], PRODUCTS[2]];
  document.getElementById("igGrid").innerHTML = set.map(p => `
...
function renderCheckout() {
  ...
  if (!box) return;
  if (!cart.length) {
    box.innerHTML = `<p style="color:var(--ink-55);font-size:14px">Your bag is empty.</p>`;
...
const COLLECTION_CARDS = [
  { n: "Bridal", c: "Lace, silk & beaded tulle for the big day", fabric: "embroidery", col: "Champagne" },
  { n: "Occasion", c: "Statement fabrics for weddings & parties", fabric: "sequin", col: "Plum" },
  { n: "Signature", c: "Our core edit — velvet, silk & chiffon", fabric: "velvet", col: "Royal" },
  { n: "Everyday", c: "Ankara, brocade & easy-wear favourites", fabric: "ankara", col: "Terracotta" }
];
function renderCollections() {
  const el = document.getElementById("collectionGrid"); if (!el) return;
  el.innerHTML = COLLECTION_CARDS.map((x, i) => `
    <a class="cat" href="shop.html?collection=${encodeURIComponent(x.n)}">
      ${swatchHTML(x.fabric, x.col)}
      <div class="cat-veil"></div>
      <span class="cat-num">${String(i + 1).padStart(2, "0")}</span>
      <div class="cat-label"><div class="n">${x.n}</div><div class="c">${x.c}</div></div>
...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  await loadProducts();                  // real products from Supabase
  renderPage(page);
